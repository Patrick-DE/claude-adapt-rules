...
from claude_adapt_rules.ledger import default_ledger_path


def test_defaults_live_under_the_user_home(monkeypatch):
    for var in ("CLAUDE_ADAPT_RULES_HOME", "CLAUDE_ADAPT_RULES_DATA_DIR", "CLAUDE_ADAPT_RULES_RULES_DIR", "CLAUDE_ADAPT_RULES_LEDGER"):
        monkeypatch.delenv(var, raising=False)
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: Path("/home/example")))

    assert extract_mod.home_dir() == Path("/home/example/.claude-adapt-rules")
    assert extract_mod.data_dir() == Path("/home/example/.claude-adapt-rules/data")
    assert extract_mod.rules_dir() == Path("/home/example/.claude-adapt-rules/rules")
    assert default_ledger_path() == Path("/home/example/.claude-adapt-rules/rules/ledger.json")
...
def test_home_override_moves_everything(monkeypatch, tmp_path):
    for var in ("CLAUDE_ADAPT_RULES_DATA_DIR", "CLAUDE_ADAPT_RULES_RULES_DIR", "CLAUDE_ADAPT_RULES_LEDGER"):
        monkeypatch.delenv(var, raising=False)
...
def test_narrower_overrides_win(monkeypatch, tmp_path):
    monkeypatch.setenv("CLAUDE_ADAPT_RULES_HOME", str(tmp_path / "state"))
    monkeypatch.setenv("CLAUDE_ADAPT_RULES_DATA_DIR", str(tmp_path / "elsewhere"))
    monkeypatch.setenv("CLAUDE_ADAPT_RULES_LEDGER", str(tmp_path / "custom.json"))
