...
every module docstring states what defect motivated it. These files carry only the
decisions that have not been built yet, plus the reasoning that should survive them.

Update the status line when a feature ships, and move the item into roadmap's **Done**
section with the version that shipped it.
